...
function initializeEventListeners() {
    ...
    if (featuresInput) featuresInput.addEventListener('input', updatePreview);
    if (specificationsInput) specificationsInput.addEventListener('input', updatePreview);
...
function sanitizeHTML(input) {
    ...
    const allowedTagsRegex = new RegExp(`<(?!\/?(${CONFIG.allowedTags.join('|')})\s*\/?>)[^>]+>`, 'gi');
